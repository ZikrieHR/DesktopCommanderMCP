 ...
 * Performance optimization: Fast-path for standard LF string (no '\r' present),
 * avoiding unnecessary regex allocations and replacements.
 */
export function normalizeLineEndings(text: string, targetLineEnding: LineEndingStyle): string {
    // Fast-path: If the input string has no carriage returns ('\r'),
    // we can avoid the multi-pass regex normalization step.
    if (!text.includes('\r')) {
        if (targetLineEnding === '\n') {
            return text;
